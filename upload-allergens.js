...
// Now update via the app's message system
async function uploadAllergens() {
  console.log('Uploading allergen data for', updatedOverlays.length, 'items...');
  
  // Get current restaurant data
  const slug = new URLSearchParams(window.location.search).get('slug');
  if (slug !== 'mama-santa-s') {
    console.error('Please run this on the Mama Santa\'s menu editor page');
    return;
  }
  
  // Use the app's send function to save
  send({
    type: 'saveOverlays',
    overlays: updatedOverlays,
    menuImage: state.restaurants.find(r => r.slug === 'mama-santa-s').menuImage,
    changes: {
      author: 'System',
      general: ['Added inferred allergen information for all 76 menu items'],
      items: {}
    }
  });
  
  console.log('✓ Upload initiated! Check for success message.');
}
